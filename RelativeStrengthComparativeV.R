RSCV <- function(lst,  label = "Crypts", freq = "daily", to_console = FALSE) {
        ...
        ##      --      Use  %in% -- lot faster 
        mydat <- filter(all_data, all_data$Name %in% lst) %>% arrange(Date)
        ...
        RscHeatmapV(df.z, label, freq, to_console)                
        return(to_graph)
        fnDyGraph(to_graph,all_data)
}
