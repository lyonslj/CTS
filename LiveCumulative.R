LiveCumulative <- function(portfolio = "LiveP", basis = "pl") {
library(readxl)
...
for(i in inst) {
        ...
        if(portfolio == "LiveP") { 
                ...
                y <- subset(alldata2,alldata2$Name == mylst[rown,1]  & alldata2$Date <= mylst[rown,7]  & alldata2$Date >= mylst[rown,2]) }   
        if(basis == "pm"){
                y$pm <- ((y[,6] - mylst[rown,3])/mylst[rown,3])*100           # change in P * Q held
                pm <- xts(y[,8],order.by = y[,2])        # convert to xts
                assign(mylst[rown,1],pm)
        } else {
                y$pl <- ((y[,6] - mylst[rown,3])*mylst[rown,5])/100   
                pl <- xts(y[,8],order.by = y[,2])        # convert to xts
