CumRetV <- function(mylst = instLive, dt = Sys.Date()-250, nm = "Cumulative Returns Top20" , pth = "Rplots/") {
        ...
        # The function first retrieves the closing prices for each instrument from a global data frame `all_data`
        ...
        for(i in mylst) {
                y <- all_data[all_data$Name == i & all_data$Date >= dt, ]
                y <- y[,c("Date","Close")]
        ...
        dp <- head(myxts,1)                             # price on day1 of set
