RscHeatmapOrdered <- function(dataset, name = "LongWatch") {
        #########################################################
        ...
        x <- paste("JSEdat$Name == ",dQuote(dataset),sep = "")
        ...
        y_reworked <- y_reworked[complete.cases(y_reworked),]
        # y_reworked <- na.omit(tail(y_reworked,150))                        # get rid of NA values
        ...
        mypath <- paste(file.path("/Users/johnlyons/Documents/Personal/DataScience/R/RPlots/Correlation/"),"
                .csv",sep="")
        df.z <- data.frame(z)  
        ###############################################################
        ### C) Transpose the data, select 30 rows, order by latest date
        ###############################################################
        
        tdf.z <- tail(df.z,40)
        tdf.z <- as.data.frame(t(tdf.z))
        #last_col <- paste("tdf.z$",tail(colnames(tdf.z),1),sep="")
        #assign(x,paste("tdf.z$",tail(colnames(tdf.z),1),sep=""))
        x <- ncol(tdf.z)
        OrdTdf.z <- tdf.z[order(tdf.z[,x]),] 
        
        
        #OrdTdf.z <- tdf.z[assign(last_col),] 
        #OrdTdf.z <- tdf.z[order(tdf.z$`2020-04-01`),] 
        #OrdTdf.z <- tdf.z[order(tdf.z$assign(last_col)),] 
        OrdTdf.z <- as.data.frame(OrdTdf.z)
        mat_data <- data.matrix(OrdTdf.z[,(ncol(OrdTdf.z)-30):ncol(OrdTdf.z)])
        #mat_data <- data.matrix(OrdTdf.z[,(ncol(OrdTdf.z)-13):ncol(OrdTdf.z)])
        ...
        png(paste("/Users/johnlyons/Documents/Personal/DataScience/R/RPlots/",name,".png",sep =""),    # create PNG for the heat map        
        ...
        dev.off()
        #graph top of the list
        to_graph <- tail(rownames(mat_data),10) ##Longs
        to_graph <- head(rownames(mat_data),10) ##Shorts
        DailyGraph(to_graph)    
